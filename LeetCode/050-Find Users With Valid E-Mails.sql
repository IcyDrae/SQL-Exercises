...
-- Solution
-- 1. Find the valid emails using regex
SELECT
    [user_id],
    [name],
    [mail]
FROM [dbo].[Users]
WHERE
-- 2. First make sure that it starts with a letter and ends
-- with '@leetcode.com'
    [mail] LIKE '[a-zA-Z]%@leetcode.com' AND
-- 3. Then ensure that the characters before '@' are all valid characters
