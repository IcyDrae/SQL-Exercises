...
logged in for at least two consecutive days starting from
their first login date, then divide that number by the total
number of players.
...
[ConsecutiveDays] AS (
    SELECT DISTINCT
        [Activity].[player_id]
    FROM
        [dbo].[Activity] AS [Activity]
    INNER JOIN [FirstLogin] AS [FirstLogin] ON
        ([Activity].[player_id] = [FirstLogin].[player_id]) AND
        DATEDIFF(DAY, [FirstLogin].[first_login], [Activity].[event_date]) = 1
)

SELECT
    COALESCE(
        ROUND(
            NULLIF(CAST(COUNT(DISTINCT [ConsecutiveDays].[player_id]) AS DECIMAL), 0)
            /
            NULLIF(CAST(COUNT(DISTINCT [Activity].[player_id]) AS DECIMAL), 0)
        , 2)
    , 0)
    AS [fraction]
FROM
