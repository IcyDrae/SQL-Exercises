...
-- Solution
-- 1. find unique number of people
WITH [User] AS (
...
-- 2. find unique number of friends
...
FROM
    [User] AS [User]
LEFT JOIN
    [dbo].[RequestAccepted] AS [RequestAccepted] ON
    [User].[id] = [RequestAccepted].[requester_id] OR
    [User].[id] = [RequestAccepted].[accepter_id]
GROUP BY [User].[id]
ORDER BY [num] DESC;
