...
-- Solution
WITH [UserConfirmation] AS (
    SELECT TOP 1000000
        [Signups].[user_id],
        [Signups].[time_stamp],
        [Confirmations].[user_id] AS [confirmation_user_id],
        [Confirmations].[time_stamp] AS [confirmation_timestamp],
        [Confirmations].[action]
    FROM [dbo].[Signups] AS [Signups]
    LEFT JOIN [dbo].[Confirmations] AS [Confirmations] ON
...
SELECT
    ...
    ROUND(
        COALESCE(
            ...
            /
            CAST(COUNT([confirmation_user_id]) AS DECIMAL),
            0
        ),
        2
    )
