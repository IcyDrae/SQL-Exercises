...
It is guaranteed that each (machine_id, process_id) pair has a
...
SELECT DISTINCT
    [Current].[machine_id],
    ROUND(
        SUM(([Previous].[timestamp] - [Current].[timestamp]))
        /
        COUNT(DISTINCT [Current].[process_id]),
        3
    )
    AS [processing_time]
FROM
    [dbo].[Activity] AS [Current]
INNER JOIN [dbo].[Activity] AS [Previous] ON
    ([Current].[machine_id] = [Previous].[machine_id]) AND
    ([Current].[process_id] = [Previous].[process_id]) AND
    ([Current].[activity_type] = 'start') AND
    ([Previous].[activity_type] = 'end')
GROUP BY [Current].[machine_id];
