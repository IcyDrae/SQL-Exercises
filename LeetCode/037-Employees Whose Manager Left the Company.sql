...
-- Solution
WITH [CurrentManager] AS (
    SELECT DISTINCT
        [Manager].[manager_id],
        [Employees].[name]
    FROM [dbo].[Employees] AS [Manager]
    INNER JOIN [dbo].[Employees] AS [Employees] ON
        ([Manager].[manager_id] = [Employees].[employee_id])
)
...
FROM
    [dbo].[Employees]
WHERE
    [manager_id] NOT IN (SELECT [manager_id] FROM [CurrentManager])
AND ([salary] < 30000)
